...
where vhf_validity_code in (1,2,3,5,6) and (utm_x is null or utm_y is null);
...
WHERE corine_land_cover_1990_code is null and vhf_validity_code in (1,2,3,5,6) and st_intersects(st_transform(geom,3035), rast);

UPDATE main.vhf_data_animals
SET corine_land_cover_2000_code = st_value(rast,st_transform(geom,3035)) 
from env_data.corine_land_cover_2000
WHERE corine_land_cover_2000_code is null and vhf_validity_code in (1,2,3,5,6) and st_intersects(st_transform(geom,3035), rast);

UPDATE main.vhf_data_animals
SET corine_land_cover_2006_code = st_value(rast,st_transform(geom,3035)) 
from env_data.corine_land_cover_2006
WHERE corine_land_cover_2006_code is null and vhf_validity_code in (1,2,3,5,6) and st_intersects(st_transform(geom,3035), rast);

UPDATE main.vhf_data_animals
SET corine_land_cover_2012_code = st_value(rast,st_transform(geom,3035)) 
from env_data.corine_land_cover_2012
WHERE corine_land_cover_2012_code is null and vhf_validity_code in (1,2,3,5,6) and st_intersects(st_transform(geom,3035), rast);
...
where
vhf_validity_code in (1,2,3,5) and 
...
where
ndvi_modis_boku IS NULL AND vhf_validity_code in (1,2,3,5) and 
st_intersects(geom, pre.rast) and 
st_intersects(geom, post.rast) and 
