...
where longitude is not null and latitude is not null AND vhf_validity_code in (1,2,3) and geom is null;
...
where vhf_data_animals.animals_id = foo.animals_id and vhf_data_animals.utm_srid is null and vhf_validity_code in (1,2,3) ;
...
WHERE corine_land_cover_1990_code is null and vhf_validity_code in (1,2,3) and st_intersects(st_transform(geom,3035), rast);

UPDATE main.vhf_data_animals
SET corine_land_cover_2000_code = st_value(rast,st_transform(geom,3035)) 
from env_data.corine_land_cover_2000
WHERE corine_land_cover_2000_code is null and vhf_validity_code in (1,2,3) and st_intersects(st_transform(geom,3035), rast);

UPDATE main.vhf_data_animals
SET corine_land_cover_2006_code = st_value(rast,st_transform(geom,3035)) 
from env_data.corine_land_cover_2006
WHERE corine_land_cover_2006_code is null and vhf_validity_code in (1,2,3) and st_intersects(st_transform(geom,3035), rast);

UPDATE main.vhf_data_animals
SET corine_land_cover_2012_code = st_value(rast,st_transform(geom,3035)) 
from env_data.corine_land_cover_2012
WHERE corine_land_cover_2012_code is null and vhf_validity_code in (1,2,3) and st_intersects(st_transform(geom,3035), rast);
...
WHERE altitude_copernicus IS NULL AND vhf_validity_code IN (1,2,3) AND animals.animals_id = vhf_data_animals.animals_id AND animals.study_areas_id = dem_copernicus.study_areas_id AND st_intersects(dem_copernicus.rast,st_transform(vhf_data_animals.geom,3035));
...
WHERE slope_copernicus IS NULL AND vhf_validity_code IN (1,2,3) AND animals.animals_id = vhf_data_animals.animals_id AND animals.study_areas_id = slope_copernicus.study_areas_id AND st_intersects(slope_copernicus.rast,st_transform(vhf_data_animals.geom,3035));
...
WHERE aspect_copernicus IS NULL AND vhf_validity_code IN (1,2,3) AND animals.animals_id = vhf_data_animals.animals_id AND animals.study_areas_id = aspect_copernicus.study_areas_id AND st_intersects(aspect_copernicus.rast,st_transform(vhf_data_animals.geom,3035));
...
WHERE forest_density IS NULL AND vhf_validity_code IN (1,2,3) AND animals.animals_id = vhf_data_animals.animals_id AND animals.study_areas_id = forest_density.study_areas_id AND st_intersects(forest_density.rast,st_transform(vhf_data_animals.geom,3035));
...
where
ndvi_modis_boku IS NULL AND vhf_validity_code = 1 and 
st_intersects(geom, pre.rast) and 
st_intersects(geom, post.rast) and 
