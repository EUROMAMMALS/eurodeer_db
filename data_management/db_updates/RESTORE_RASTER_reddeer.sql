 ...
 SELECT row_number() OVER ()::integer AS id,
   ...
   FROM ( SELECT clc12_version_18_5.code_12 AS clc_code,
           ...
           FROM data.clc12_version_18_5,
            reddeer.temp_study_areas_mcp_individuals study_areas
...
raster2pgsql -c -R -F -C -I -x -t 128x128 -N 255 -M E:\eurodeer_data\raster\remote_sensing\forestcover\copernicus\local_data\forest_density_red_*.tif env_data.forest_density| psql -d eureddeer_db -U postgres  -p 5432
...
E:\PostgreSQL\9.5\bin\raster2pgsql.exe -c -R -F -C -I -x -t 128x128 -N 32768 -M E:\eurodeer_data\raster\dem\copernicus\local_data\dem_red*.tif env_data.dem_copernicus | E:\PostgreSQL\9.5\bin\psql.exe -d eureddeer_db -U postgres  -p 5432
E:\PostgreSQL\9.5\bin\raster2pgsql.exe -c -R -F -C -I -x -t 128x128 -N -9999 -M E:\eurodeer_data\raster\dem\copernicus\local_data\aspect_red*.tif env_data.aspect_copernicus | E:\PostgreSQL\9.5\bin\psql.exe -d eureddeer_db -U postgres  -p 5432
E:\PostgreSQL\9.5\bin\raster2pgsql.exe -c -R -F -C -I -x -t 128x128 -N -9999 -M E:\eurodeer_data\raster\dem\copernicus\local_data\slope_red*.tif env_data.slope_copernicus | E:\PostgreSQL\9.5\bin\psql.exe -d eureddeer_db -U postgres  -p 5432
...
CREATE FOREIGN TABLE analysis.gps_update_env_fields_reddeer
   (gps_data_animals_id integer ,
    ...
    snow_modis integer )
   SERVER eureddeer_fdw
   OPTIONS (schema_name 'main', table_name 'view_gps_update_env_fields_reddeer');
-- I create the phisical table to store the list (and update the values)
CREATE TABLE env_data.reddeer_update_gps_update_env_fields
(
  gps_data_animals_id integer,
  acquisition_time timestamp with time zone,
  geom geometry(Point,4326),
...
ALTER TABLE env_data.reddeer_update_gps_update_env_fields
  ADD CONSTRAINT reddeer_update_gps_update_env_fields_pkey PRIMARY KEY(gps_data_animals_id);
CREATE INDEX reddeer_update_gps_update_env_fields_geom_gist
  ON env_data.reddeer_update_gps_update_env_fields
  USING gist
  (geom);
CREATE INDEX reddeer_update_gps_update_env_fields_geom3035_gist
  ON env_data.reddeer_update_gps_update_env_fields
  USING gist
...
CREATE FOREIGN TABLE env_data.reddeer_update_gps_update_env_fields
   (gps_data_animals_id integer ,
    acquisition_time timestamp with time zone ,
    geom geometry(point,4326) ,
    gps_validity_code smallint ,
   ...
   SERVER eurodeer_fdw
   OPTIONS (schema_name 'env_data', table_name 'reddeer_update_gps_update_env_fields');
