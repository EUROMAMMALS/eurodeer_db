-- Wrong tags 
SELECT animals_id, latitude, longitude, geom FROM main.gps_data_animals 
WHERE longitude is null and geom is not null;
SELECT animals_id, latitude, longitude, geom FROM main.gps_data_animals 
...
-- gps_validity_code is 11 but utm_x and utm_y is provided  
SELECT * from main.gps_data_animals where gps_validity_code in (11) and utm_x is not null;
-- UPDATE main.gps_data_animals SET utm_x = NULL, utm_y = NULL WHERE gps_validity_code in (11) and utm_x is not null; 

-- check if utm_x is consistent with the geom
SELECT animals_id, latitude, longitude, geom, gps_validity_code FROM main.gps_data_animals 
WHERE utm_x != st_x(st_transform(geom,utm_srid)) and gps_validity_code = 1;
SELECT animals_id, latitude, longitude, geom, gps_validity_code FROM main.gps_data_animals 
WHERE utm_y != st_y(st_transform(geom,utm_srid)) and gps_validity_code = 1;
